...
стоит в `APP_ROOT` (`/etc/victory-backup/backup.env`) и в `ExecStart`
(`deploy/systemd/victory-backup-*`), а systemd не знает ни cwd, ни `~`. Разворачиваете в
другой каталог — меняйте все три места вместе, иначе бэкап после восстановления молча будет
смотреть не туда.
