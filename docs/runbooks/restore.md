...
⚠️ Абсолютный `/home/q/victory` ниже — **намеренно, не забытый хардкод**: это же значение
стоит в `APP_ROOT` (`/etc/victory-backup/backup.env`), откуда его читает `bin/backup`,
запускаемый systemd от root — ни cwd, ни `~` там не работают (`~` дал бы `/root`).
Разворачиваете в другой каталог — меняйте и `APP_ROOT`, иначе бэкап после восстановления
молча будет смотреть не туда. Сами юниты путь чекаута не содержат: `ExecStart` у них —
`/usr/local/bin/victory-backup`, куда `bin/backup` **копируется** (см. «Установка и
обновление скрипта»).
...
### По чему выносится приговор (с 10.09.26)

Не по коду возврата `pg_restore`, а по его логу. Код возврата обманывает в обе
стороны: он ненулевой на безобидных различиях окружения и при этом **не** ловил
частично неудавшееся восстановление — 06.09.26 не создался `index_users_on_phone`,
в логе стояло «errors ignored on restore: 1», а таймер отрапортовал успехом, и
проверка врала зелёным четвёртые сутки.

Поэтому теперь:

* строка `errors ignored on restore: N` разбирается явно; при `N > 0` каждая
  ошибка сверяется со списком безобидных;
* безобидными считаются ровно две: `role "…" does not exist` и `must be owner of`
  — одноразовый контейнер поднимается без наших ролей. **Всё остальное валит
  проверку**, включая то, что раньше проглатывалось молча;
* ошибки `gpg` идут в тот же лог, что и `pg_restore`. Без этого неверная
  парольная фраза выглядела как «input file is too short» — красно правильно, но
  причина не названа.

### Где искать подробности, когда проверка провалилась

Сообщение об ошибке печатает до девяти строк опасных ошибок (с `DETAIL`, где
лежит конкретный ключ-дубликат) и путь к **датированному слепку** лога:

```
/var/backups/victory/tmp/verify-restore-<dd.MM.yy-HHMM>.log
```

Слепок нужен потому, что рабочий `verify-restore.log` лежит по фиксированному
пути и затирается следующим прогоном — а следующий прогон вы запустите сразу,
на копии постарше. Два провала внутри одной минуты не сливаются в один файл:
имя получает суффикс `-2`, `-3` и так далее. Слепки не ротируются — они
появляются только на провалах, и каждый из них единственная улика по своему.

При запуске из таймера подробности уезжают в
`/var/backups/victory/logs/weekly.log` (туда systemd шлёт stderr юнита);
в Telegram приходит только строка «не сделано: проверка-восстановления».
...
| 10.09.26 | `verify` на дампе с 400 опасными ошибками (лог 160 КБ) | код 1, сообщение с выдержкой и слепком напечатано |
| 10.09.26 | `verify` с неверной парольной фразой | код 1, в логе названа причина от `gpg`, а не только «input file is too short» |
| 10.09.26 | `verify` на дампе с безобидными ошибками (политика RLS на несуществующую роль) | код 0, «все они из списка безобидных» |
