...
для этого нет (см. выгрузку API вне репозитория, на прод-хосте `/home/q/document_pdf.md`, секция «Получать id карточек объектов,
...
- Документация API: `.claude/docs/topnlab/` (выжимки в репозитории, поиск — `rake topnlab_docs:search[query]`); первоисточник — выгрузка `document_pdf.md` вне репозитория (на прод-хосте `/home/q/document_pdf.md`)
