...
для этого нет (см. выгрузку API `document_pdf.md` вне репозитория, секция «Получать id карточек объектов,
...
- Документация API: `.claude/docs/topnlab/` (выжимки в репозитории, поиск — `rake topnlab_docs:search[query]`); первоисточник — выгрузка `document_pdf.md` вне репозитория
