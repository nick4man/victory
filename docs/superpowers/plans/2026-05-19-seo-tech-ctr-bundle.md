...
```bash
mkdir -p ~/victory/.claude/docs/seo
```
...
Run: `ls ~/victory/app/helpers/blog_helper.rb 2>&1`
...
```bash
cd ~/victory && head -60 app/views/shared/_jsonld_article.html.erb
```
...
```bash
cd ~/victory && grep -n 'content_for :head\|jsonld_article' app/views/blog/show.html.erb | head
```
...
```bash
cd ~/victory && cat app/controllers/blog_controller.rb
```
...
```bash
cd ~/victory && cat app/views/blog/index.html.erb
```
...
```bash
cd ~/victory && grep -n 'blog_url\|articles_mod' app/views/sitemap/pages.xml.erb | head
```
