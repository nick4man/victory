...
| 14 | 61 | **Photo «передать сотруднику без задачи»**: 4-кнопочный prompt (cloud/share/task/cancel). `dispose:share` → staff picker → `share_caption` step (опц. подпись или `/skip`) → `PhotoShareContinuation` шлёт sendPhoto с `file_id` к staff.dm_chat_id + NC archive (30 дн). Task НЕ создаётся (главное отличие от task-flow). Legacy alias `dispose:staff` → task. + **DM ack UX**: `DmQnaHandler` шлёт «💭 Принял, думаю…» до LLM-вызова + edit-in-place финальный ответ (отдельный коммит). | — |
| 15 | — | **Director DM control panel** — DM с @anvictorybot становится полноценной admin panel. 4 pillars: (1) READ — FTS-поиск group messages (`telegram_group_messages` table + tsvector russian + `search_group_messages` tool) + cross-staff search (`search_all_tasks`, `search_all_leads`) + agency_wide `director_self_audit` + top/bottom/anomalies `kpi_for`. (2) WRITE — `Commands::Base#resolve_lead!` (anchor_or_id), 6 команд DM-friendly (/assign /route /stage /note /close /task с lead_id первым arg). (3) PUSH — `DailyDigestJob` (08:30 MSK) + `WeeklySummaryJob` (пн 10:00 MSK) — DM директорам с inline-кнопками. (4) PANEL — `/dashboard` команда + `DirectorDashboard` service (markdown snapshot 4 секций) + `DashboardDrillCallback` (drilldown по [🎯][✅][👥][🔍] кнопкам). Docs: `.claude/docs/director-control-panel.md`. Backfill: `rake telegram:backfill_group_messages`. | — |
| 15.5 | — | **LLM summarization + cabinet polish**: новый `ChatTools::Staff::SummarizeGroupMessages` — LLM сжимает обсуждения в 3-секционное markdown (📌 темы / ❓ открытые вопросы / ✅ решения). Композиция в `DailyDigestJob` — director получает «💬 Обсуждалось в рабочем чате» секцию автоматически + cifры. `/cheatsheet` команда (pinned reference card). `/assign <lead_id>` без username → inline picker. `Help::ENTRIES` обновлён DM-форматами. `/cabinet/documents` unified view (consents + scans + valuations). Pillar 1 banner-CTA «📲 Подключи Telegram» для unlinked клиентов. KPI snapshot honest (active LLM-meta 24/24=100%, cabinet shipped + TG-link rate). Spec coverage: IntentClassifier + TextIntakeProcessor + StaffChatResponder + Topnlab webhook + Dashboard + list_my_open_tasks id:N + summarize. CF Worker fix deployed (defense-in-depth secret_token). | — |
