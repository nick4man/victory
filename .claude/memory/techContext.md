...
Пути внутри репозитория приведены к относительным — 161 вхождение. Остались **86
абсолютных `/home/q/...` в 22 файлах, и это не недоделка**: у каждого нет точки отсчёта
внутри репозитория. Проверено построчно, конвертируемых среди них нет.
...
| Файлы намеренно вне чекаута | 21 | `db-baseline.txt`, `db-rollback.yml`, `document_pdf.md`, выгрузка API Topnlab. Они и должны лежать снаружи, чтобы переживать `reset --hard` |
...
логический `pwd` вернёт путь через симлинк, и guard прома́хнётся.
