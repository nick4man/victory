...
Эталон, снятый заранее, разъехался бы с послеоконным сам по себе — от исправно
...
```
Все три SQL-проверки — те же запросы, что в эталоне, снятом внутри окна сразу
после шага 4: сравниваем вывод с `/home/q/db-baseline.txt`, а не с ожиданием
«строк должно быть много».
```sql
