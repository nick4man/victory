...
отслеживаемым ~10 месяцев в **публичном** репозитории. `.gitignore` называл его
с самого начала (блок «never commit secrets»), но на уже отслеживаемые файлы
ignore не действует — отсюда вечное `M .env.production` в `git status` на проде.
...
| `DATABASE_PASSWORD` | 35 | сверить с боевым; БД слушает только внутри docker-сети |
...
```bash
cd ~/victory
cp .env.production ~/env.production.$(date +%F).bak   # страховка
git pull --ff-only origin main
ls .env.production || echo 'файл удалён — это ожидаемо, он не используется'
```
