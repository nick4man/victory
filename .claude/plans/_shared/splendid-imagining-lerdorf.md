...
Над репо victory62 работают 4 параллельные Claude Code сессии. Сейчас chat/seo/upgrade имеют worktree, но сессия **victory делит main checkout** — а это **live-prod bind-mount**: контейнер `victory-web-1` монтирует main checkout в `/app` в `RAILS_ENV=development` с code-reload, поэтому **любая правка там мгновенно попадает на живой сайт**. Это и коллизия между сессиями, и прямой риск для прода.

Цель: каждая сессия — в своём чистом worktree на ветке `dev/<session>` от текущего main (Rails 8.1.3.1); main checkout зарезервирован **только под deploy/merge** (никакой активной разработки). Идентичность сессии — автоматически через marker-файл. Тулинг (inbox / lock / hooks / доки) — полностью на 4 сессии.

## Текущее состояние (проверено, read-only)

| Worktree | Ветка | Позади main | Впереди | Чисто |
|---|---|---|---|---|
| main checkout | **main** (74ba0f2) | — | — | ✓ (prod bind-mount) |
...
```
<main checkout>     main         prod bind-mount + deploy/merge ONLY (live code-reload — НЕ трогать!)
...
5. Положить `.claude-session` в корень каждого worktree:
   ...
   - `.claude-session` → `main` (маркер «reserved checkout» — hook предупредит «здесь только deploy/merge»).
...
7. Доработать `.claude/hooks/session-start.sh` (около строки 16, `SESSION_ID="${CLAUDE_SESSION:-unknown}"`):
   ...
   - **guard**: если `SESSION_ID == main` (или worktree = main checkout) — крупное предупреждение «prod bind-mount, только deploy/merge»; если идентичность не совпадает с ожидаемым маркером worktree — предупредить о запуске в чужом месте.
...
12. Обновить `.claude/sessions/README.md`, `.claude/skills/session-coordination/SKILL.md`, `.claude/agents/session-coordinator.md`:
    ...
    - выделить предупреждение **main checkout = live-prod bind-mount, только deploy/merge, НИКОГДА не активная разработка**;
...
### E. Carry-over
14. Старое inbox-сообщение в main checkout (`.claude/sessions/inbox/victory/`, одно, от 04.06) — inbox per-worktree, поэтому при желании скопировать в `victory-victory/.claude/sessions/inbox/victory/`; вероятно устарело — можно оставить/архивировать.

## Verification (end-to-end)
- `git worktree list` → 5 записей; каждая сессия на `dev/<session>`, main checkout на `main`.
...
- Все изменения тулинга (inbox/lock/hook/доки) правятся в **одном** worktree (например `victory-victory` или напрямую в рабочем), коммитятся в `dev/<session>` → PR → main как обычно; main checkout получит их только при следующем deploy-обновлении (не срочно, тулинг работает из любого worktree через shared `.git`).
