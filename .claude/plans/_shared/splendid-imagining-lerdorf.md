...
Над репо victory62 работают 4 параллельные Claude Code сессии. Сейчас chat/seo/upgrade имеют worktree, но сессия **victory делит `~/victory`** — а это **live-prod bind-mount**: контейнер `victory-web-1` монтирует `~/victory → /app` в `RAILS_ENV=development` с code-reload, поэтому **любая правка там мгновенно попадает на живой сайт**. Это и коллизия между сессиями, и прямой риск для прода.

Цель: каждая сессия — в своём чистом worktree на ветке `dev/<session>` от текущего main (Rails 8.1.3.1); `~/victory` зарезервирован **только под deploy/merge** (никакой активной разработки). Идентичность сессии — автоматически через marker-файл. Тулинг (inbox / lock / hooks / доки) — полностью на 4 сессии.

## Текущее состояние (проверено, read-only)

| Worktree | Ветка | Позади main | Впереди | Чисто |
|---|---|---|---|---|
| `~/victory` | **main** (74ba0f2) | — | — | ✓ (prod bind-mount) |
| `~/victory-chat` | dev/chat | 34 | **0** | ✓ (нет remote) |
| `~/victory-seo` | dev/seo | 34 | **0** | ✓ (нет remote) |
| `~/victory-upgrade` | upgrade/rails-8-eol-phase2 | — | 2 (уже в main через PR #8) | ✓ |
...
```
~/victory           main         prod bind-mount + deploy/merge ONLY (live code-reload — НЕ трогать!)
~/victory-victory   dev/victory  NEW — сессия victory
~/victory-chat      dev/chat     refresh → main
~/victory-seo       dev/seo      refresh → main
~/victory-upgrade   dev/upgrade  switch с rails-8-ветки, refresh → main
```
...
1. Освежить существующие session-worktree до main (все чистые, 0 впереди → ff-only, без потерь):
   - `git -C ~/victory-chat merge --ff-only origin/main`
   - `git -C ~/victory-seo  merge --ff-only origin/main`
2. Починить upgrade-worktree: перевести на `dev/upgrade` + освежить:
   - `git -C ~/victory-upgrade checkout dev/upgrade`
   - `git -C ~/victory-upgrade merge --ff-only origin/main`
...
5. Положить `.claude-session` в корень каждого worktree:
   ...
   - `~/victory/.claude-session` → `main` (маркер «reserved checkout» — hook предупредит «здесь только deploy/merge»).
...
7. Доработать `.claude/hooks/session-start.sh` (около строки 16, `SESSION_ID="${CLAUDE_SESSION:-unknown}"`):
   ...
   - **guard**: если `SESSION_ID == main` (или worktree = `~/victory`) — крупное предупреждение «prod bind-mount, только deploy/merge»; если идентичность не совпадает с ожидаемым маркером worktree — предупредить о запуске в чужом месте.
...
12. Обновить `.claude/sessions/README.md`, `.claude/skills/session-coordination/SKILL.md`, `.claude/agents/session-coordinator.md`:
    ...
    - выделить предупреждение **`~/victory` = live-prod bind-mount, только deploy/merge, НИКОГДА не активная разработка**;
...
### E. Carry-over
14. Старое inbox-сообщение в `~/victory/.claude/sessions/inbox/victory/` (одно, от 04.06) — inbox per-worktree, поэтому при желании скопировать в `victory-victory/.claude/sessions/inbox/victory/`; вероятно устарело — можно оставить/архивировать.

## Verification (end-to-end)
- `git worktree list` → 5 записей; каждая сессия на `dev/<session>`, `~/victory` на `main`.
- В каждом worktree `cat .claude-session` = ожидаемое имя.
- Прогнать hook вручную без env: `CLAUDE_PROJECT_DIR=~/victory-chat bash ~/victory-chat/.claude/hooks/session-start.sh` → печатает `session=chat` + путь worktree, без mismatch-warning; тот же прогон в `~/victory` → предупреждение «prod bind-mount / deploy-only».
...
- `git -C ~/victory branch --show-current` → `main` (не трогали); **прод HTTP 200** (checkout `~/victory` не менялся — только добавлен gitignored `.claude-session`).

## Risks / mitigations
- **Нельзя трогать checkout `~/victory`** (live prod). Все refresh — на ДРУГИХ worktree. В `~/victory` добавляется только gitignored `.claude-session=main` (ноль влияния на код/reload).
...
- Все изменения тулинга (inbox/lock/hook/доки) правятся в **одном** worktree (например `victory-victory` или напрямую в рабочем), коммитятся в `dev/<session>` → PR → main как обычно; `~/victory` получит их только при следующем deploy-обновлении (не срочно, тулинг работает из любого worktree через shared `.git`).
