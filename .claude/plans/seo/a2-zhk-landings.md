...
## Данные каталога — замер 09.08.26 (MCP postgres, read-only)

Планировался вслепую: из seo-worktree нет `bundle`, а MCP postgres подключился только после Фазы 1. Замер меняет часть решений, поэтому фиксируется здесь.

| Метрика | Значение |
|---|---|
| `properties` не удалённых | 117 |
| из них `on_site` (status=active + published_at) | **18** |
| archived | 82 (48 из них — 09.08.26, 04:00 UTC) |
| draft | 17 |
| `city IS NULL` | **0** |
| `geom IS NULL` | **0** |
| `district IS NULL` | 61 из 117; **12 из 18** `on_site` |

Объекты по микрорайонам (`on_site`): Дашково-Песочня — 2, Приокский — 1, **остальные 18 микрорайонов — 0**. Плюс 2 под админ-именем «Московский», 1 «Рязанский».

Следствия, уже учтённые ниже:

1. **`indexable?` = «текст ИЛИ объекты» — не запас прочности, а единственный способ запустить слой.** При правиле «только с объектами» индексируемых ЖК было бы ноль. Гейт Фазы 3 достижим исключительно редакторским текстом.
2. **Привязка объектов — по координатам, не по адресу.** `geom` заполнен у 100% строк, адреса же непригодны: у 12 из 18 `on_site` нет района, и backfill их не спасёт — проверено, 0 из 12 содержат в адресе хоть один алиас из `RyazanDistricts`. Типичный адрес — «Рязанская обл., ул. Малиновая, д. 4».
3. **Риск «`city` может быть NULL» снят фактом** — 0 строк. Backfill города не нужен.
4. **`district` содержит значения вне реестра** — «Рязанский», «Белореченский», «Ломоносовский» (объекты других городов). Валидация ЖК от этого защищена, suggester не должен об это спотыкаться.
5. **Порядок наполнения ЖК** (по наличию инвентаря): Скобелев / Открытие / Старое Село 2 (ДП, 2 объекта) → Приокский парк (1) → Легенда, Видный, Метропарк (0, но текст всё равно даёт индексируемую страницу).

Отдельно, вне scope A2, но зафиксировать: 48 объектов ушли в архив 09.08.26 в 04:00 UTC **не** из-за `archive_missing` — во всех прогонах `topnlab_sync_runs.archived = 0` при стабильном `ids_seen = 81`, то есть статус пришёл из CRM обычным upsert'ом. При этом прогон 05:00 (run 2647) вернулся `partial` с «12 fetch error(s) — sweep incomplete, archive skipped»: гард PR #6 отработал. Владелец темы — сессия, ведущая Topnlab.

**Главный стратегический вывод:** узкое место A2 — не количество страниц, а инвентарь. 20 написанных district-лендингов обслуживают 3 объекта, 18 из них сидят в `noindex,follow`. Следующий по эффективности рычаг — наполнение каталога либо решение по `ExternalListing` (инфраструктура готова, `YRL_FEED_URLS` пуст — отложенное D2), а не новые URL.

> Единственный экземпляр этого документа — здесь. `~/.claude/plans/` — глобальный буфер с переиспользуемыми автоименами, где мы уже потеряли два документа; ручную копию там не ведём (хук `plan-sync.sh` всё равно зеркалит буфер в `.claude/plans/<session>/`, и вторая копия породила бы дубль с автоименем).
...
Жёсткое разделение сред: **seo-worktree** (`~/victory-seo`, нет bundle/rails/rspec) пишет текст файлов; **victory-worktree** (`~/victory-victory`) прогоняет всё, что требует рантайма. `~/victory` (live-prod bind-mount) и чужие worktree не трогаем. Хендофф — `bin/claude-inbox` по `.claude/skills/session-handoff-protocol`, одно сообщение на границу фазы.

Каждая фаза — отдельный PR в `main`, CI (rubocop + brakeman + bundler-audit) + обязательный `code-reviewer` по diff.

---

### Фаза 1 — слой данных (публичного эффекта нет) → PR #1 ✅ написана, [PR #11](https://github.com/nick4man/victory/pull/11)

Статус на 09.08.26: код написан и закоммичен (`c61b427`), PR открыт. Не закрыто — всё, что требует рантайма: `db:migrate`, дамп `structure.sql`, `rspec`, `rubocop`, двойной `rake zhk:seed`, проверка обратимости и живая проверка, что привязка переживает Topnlab-синк. Это victory-сессия.

Отступление от плана при реализации: индекс на `properties.residential_complex_id` сделан обычным, а не `CONCURRENTLY` — в таблице 117 строк, а `disable_ddl_transaction!` стоил бы атомарности миграции.
...
- `app/services/zhk/attachment_suggester.rb` + `rake 'zhk:suggest[legenda]'` — read-only, ничего не пишет. Порядок критериев задан замером 09.08.26:
  1. **Радиус от координат ЖК** — `Property.on_site.within_radius(lat, lng, 0.3)` (`property.rb:383`, ST_DWithin по `geom`). Первичный и практически единственный рабочий: `geom` заполнен у 100% строк.
  2. `address ILIKE` по `address_patterns` — вторичный, для ЖК без координат.
  3. Фильтр по району — только как дополнительное сужение, не как основа: у 12 из 18 `on_site` объектов `district` пуст. Если применяется — **обязательно** через `RyazanDistricts.aliases_for(slug)`, прямой `where(district: slug)` не сработает никогда (в колонке free-text алиас).

  Suggester обязан переживать значения `district` вне реестра («Рязанский», «Белореченский») — не падать, а игнорировать.
...
**Гейт мержа:** ≥3 ЖК на проде с `sitemap_ready == true`. По замеру 09.08.26 достижим **только редакторским текстом** — инвентаря на это не хватит (2 объекта в ДП, 1 в Приокском, ноль в остальных 18 микрорайонах). Перед мержем — прогон `.claude/skills/victory-seo-checklist` (10 пунктов) по обоим шаблонам.
...
3. **Тонкие страницы / soft-404.** Риск повышен фактами замера: почти все ЖК стартуют с нулём объектов, то есть страница держится только на редакционном тексте. `published` по умолчанию false; `sitemap_ready` требует `body_html`; guard и sitemap используют одно выражение; hub перечисляет только `sitemap_ready`; старт ограничен 5-7 ЖК с рукописным текстом. Минимальная планка текста — не абзац-заглушка, а ≥2 уникальных проверяемых факта (см. риск 4).
...
9. **Инвентарный голод — главное ограничение всего A2.** 18 объектов `on_site` на 20 написанных district-лендингов; 18 из них уже сидят в `noindex,follow`. ЖК-слой это ограничение не снимает — он лишь добавляет entity-страницы, которые ранжируются на собственном тексте. Если каталог не наполняется, следующий по эффективности шаг — не Фаза 4 и не новые URL, а решение по `ExternalListing` (инфраструктура готова, `YRL_FEED_URLS` пуст — отложенное D2 от 18.05.26). Переоценить после Фазы 3 по отчёту Я.Вебмастера.
