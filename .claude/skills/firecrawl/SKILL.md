...
## Project-specific note (victory62)

В этом репо Firecrawl уже интегрирован в `app/services/firecrawl/client.rb`
(Path B, REST-API через Faraday) для valuation pipeline. См.
`app/services/valuations/websearch_comp_finder.rb` для production usage
pattern: Tavily search → Firecrawl scrape → Sonnet parse → ExternalListing
persist.

ENV-key хранится как `FIRECRAWL_API_KEY` в `.env` (gitignored). Если
ключ не задан — `Firecrawl::Client.new` raises `Firecrawl::Client::Error`,
WebsearchCompFinder gracefully returns 0 без crash оценки.
