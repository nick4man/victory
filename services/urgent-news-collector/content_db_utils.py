...
def get_party_line(topic_text, limit: int = 4, min_similarity: float = 0.08):
    ...
    Фолбэка нет: если ничего не прошло по порогу, возвращается пустой список и
    промпт идёт без «прежних позиций». На коротких topic_text это штатный
    исход — триграммы там неустойчивы. (Докстринг раньше обещал «минимум один
    свежий пост», которого в запросе никогда не было.)
    """
    conn = _connect()
