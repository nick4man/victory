...
Владелец всех файлов службы — victory, включая `pipeline_utils.py` и `content_db_utils.py`.
Прежний запрет «эти два файла здесь не править» снят 07.09.26 вместе с вендорингом:
репозиторий openclaw стал архивом, который мы разбираем, а не апстримом.

Владение и границы — `SERVICE.md`. История решения — `VENDOR.md`. Выкладка в боевой каталог:

```bash
./sync-check.sh           # отчёт: отстала ли боевая копия, exit 1 если да
./sync-check.sh --deploy  # выложить наши файлы в боевой каталог
```

Забирать оттуда нечего: каталог `/opt/.openclaw/…/workspace-conveyor/IT/scripts` — это
продакшен, откуда крон запускает конвейер, а не второй источник правды.

Восемь скриптов конвейера, живущих в архиве, импортируют оба общих модуля и после
выкладки работают с нашей копией. Правя их, помни: тестов victory на них нет.
