...
module Zhk
  ...
  class AttachmentSuggester
    ...
    # Жалуемся только на то, что реально мешает выбранной стратегии. На
    # полностью настроенной карточке плашка из двух претензий научила бы
    # оператора её не читать.
    def notes_for(chosen)
      case chosen
      when 'radius', 'all'
        []
      when 'patterns'
        ['У ЖК не заданы координаты — поиск по радиусу недоступен, работаем по адресным паттернам.']
      when 'district'
        ['Ни координат, ни адресных паттернов — подбираем по району. Район заполнен не у всех объектов ' \
         'каталога: те, у кого он пуст, сюда не попадут, ищите их через «все непривязанные».']
      else
        ['Для подбора нет ни одного сигнала: не заданы ни координаты, ни адресные паттерны, ни район. ' \
         'Заполните широту и долготу в карточке или переключитесь на «все непривязанные».']
      end
    end
  end
end
