...
module Valuations
  ...
  class AiCompFilter
    ...
    SYSTEM_PROMPT_TEMPLATE = <<~PROMPT.freeze
      ...
      {"suitable": true|false, "reason": "1-2 предложения по-русски", "adjustment": -25..+25}
      ...
      ВАЖНО: для тонких рынков (мало объявлений в районе) принимать cross-district
      аналоги с поправкой ПРЕДПОЧТИТЕЛЬНЕЕ чем полный reject. Размер выборки
      статистически важнее, чем точная district-match.

      Шкала поправок по различиям:
      - тот же район, схожая площадь, разница в этаже/годе/ремонте — adjustment ±0..±10
      - соседний район (≤ 2 км или общая транспортная зона) — adjustment ±5..±10
      - другой район в том же городе/округе — adjustment ±10..±20
      - другая эпоха застройки (хрущёвка vs новостройка) — adjustment ±15..±25
      - другой city-сегмент (центр vs окраина при разных рынках) — adjustment ±20..±25

      Не подходят как аналоги (suitable: false):
      - принципиально иной сегмент при одинаковой площади (студия 50 м² vs 2-комн 50 м²)
      - совсем другой город или regional market
      - non-residential для residential subject (склад vs квартира)
      - закрытое/архивное объявление (если видно из source)
    PROMPT
