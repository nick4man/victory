...
module AuditPdf
  ...
  class EiDetailsPage
    ...
    def ei_table
      ...
      headers = ['Стратегия', 'EI median', 'Monte Carlo p5–p95', 'P(BUY)']
      rows = [
      ...
      @doc.move_down 6
      @doc.fill_color Theme::MUTED
      @doc.text '◆ — стратегия, рекомендованная по итогам Monte Carlo (1 000 000 симуляций).',
                size: 8
      @doc.fill_color Theme::INK
      @doc.move_down 14
    end
    ...
    def ei_explainer
      explainer(
        ...
        'P(BUY) показывает вероятность остаться в плюсе при тысячах разных вариантов будущего.'
      )
    end
    ...
    def mortgage_explainer
      ...
      explainer(
        ...
        'плюс остаточный долг, а не всю переплату за 20 лет. Снижение ставки на 1% существенно меняет EI (см. стр. 3).'
      )
    end
