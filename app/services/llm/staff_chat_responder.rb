  ...
  class StaffChatResponder
    ...
    def system_prompt
      ...
      <<~PROMPT.strip
        ...
        TOOL-ERROR HANDLING:
          • Если tool вернул {error: "tool_failed", message: "..."} — НЕ
            повторяй тот же tool с теми же args. Попробуй ОДИН альтернативный
            вариант (расширить period, поменять filter) либо извинись.
          • Если tool вернул {summary: null, note: "..."} — ОТВЕТЬ TEXT note
            ДОСЛОВНО, не зови tool ещё раз. Note специально написан для пользователя.
          • Никогда не извиняйся от себя «функция не может» — это путает пользователя.
            Используй точный message из tool result.
      PROMPT
    end
