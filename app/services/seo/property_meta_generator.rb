...
module Seo
  class PropertyMetaGenerator
    ...
    SYSTEM_PROMPT = <<~PROMPT.freeze
      Ты SEO-копирайтер российского агентства недвижимости АН «Виктори»
      (офис в Рязани, работаем в нескольких городах РФ). Ты пишешь
      meta-теги для карточки объекта недвижимости так, чтобы они хорошо
      ранжировались в Яндекс и Google и кликались юзером.
      ...
      • Город объекта определяется ИСКЛЮЧИТЕЛЬНО полем «Город» из данных
        ниже. Если объект в Москве — пиши "в Москве", не "в Рязани".
        Если в Санкт-Петербурге — "в Санкт-Петербурге". Не упоминай
        Рязань если объект в другом городе — это будет грубой ошибкой.
    ...
    def property_payload
      ...
      # Город — отдельной строкой ПЕРЕД районом, потому что system prompt
      # привязывает геофразу ("в <Город>") именно к этому полю. Без этого
      # LLM может вытащить "Рязань" из бренд-контекста системного промпта
      # и применить её к МСК/СПб объекту.
      lines << "Город: #{@property.address_locality}"
