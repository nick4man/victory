...
module Crm
  ...
  class ContactParser
    ...
    # пробелами.
    #
    # Границы (?<![\d+]) и (?!\d) обязательны. Без левой границы из длинной
    # цифровой последовательности вырезался бы десятизначный хвост: в строке
    # «ИНН 771234567890, тел 9001234567» кандидат от ИНН стоит раньше, был бы
    # взят первым, а настоящий телефон отброшен — клиент завёлся бы с чужим
    # номером. Так же с номерами счетов.
    PHONE_RE = /(?<![\d+])(?:\+?[78][\s\-()]*)?\(?\d{3}\)?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}(?!\d)/
