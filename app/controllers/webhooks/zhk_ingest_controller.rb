...
module Webhooks
  ...
  class ZhkIngestController < ApplicationController
    # CSRF-защита снята намеренно, как у всех шести соседних вебхуков.
    # Она адресует одну угрозу: браузер жертвы шлёт запрос, автоматически
    # прикладывая куки сессии. Здесь красть нечего — эндпоинт куки не
    # использует вовсе, а требует заголовок `Authorization: Bearer`,
    # который межсайтовая форма поставить не может.
    #
    # CodeQL помечает эту строку как `rb/csrf-protection-disabled` (high).
    # Предупреждение механически верное и по существу неприменимое; на
    # соседних вебхуках оно молчит только потому, что для PR показываются
    # лишь НОВЫЕ срабатывания, а те строки старые.
