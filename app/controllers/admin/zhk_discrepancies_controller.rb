...
module Admin
  ...
  class ZhkDiscrepanciesController < ApplicationController
    ...
    # Русские подписи для `ZhkFact#field`. Список совпадает с
    # `Zhk::FactApplier::FILLABLE`, и сегодня в базу иначе не попасть:
    # единственный пишущий путь — `Zhk::Ingest#fields`, который режет
    # входящий payload по `FactApplier::FILLABLE.map(&:to_s)` ДО того, как
    # `record_facts` создаст `ZhkFact`. Но это свойство ровно ЭТОГО
    # вызывающего пути, а не invariant модели: `ZhkFact` валидирует
    # `field` только на presence (обычная строковая колонка), и любой
    # другой писатель — прямой `ZhkFact.create!`, консоль, будущий сид —
    # гарантию не унаследует. `Zhk::Discrepancies.all` тоже её не
    # проверяет: она строит очередь из сырых `ZhkFact`, группируя по
    # `[complex_id, field]`, независимо от того, применил ли `FactApplier`
    # это поле к карточке. Фолбэк на сырое имя здесь — страховка на
    # рассинхронизацию двух независимо поддерживаемых списков
    # (`FIELD_LABELS` и `FILLABLE`), а не теоретическая невозможность:
    # уберут поле из `FILLABLE` при переименовании/деприкейшне — старые
    # строки `ZhkFact` с этим именем в базе останутся, и без фолбэка
    # экран на них упал бы. Формулировки — как редактор видит их в самой
    # карточке ЖК (`admin/residential_complexes/_form.html.erb`), а не
    # буквальный перевод символа поля.
    FIELD_LABELS = {
      'name' => 'Название',
      'district_slug' => 'Район',
