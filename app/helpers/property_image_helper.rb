...
module PropertyImageHelper
  ...
  #
  # nil, если блоб ещё не проанализирован: ActiveStorage считает
  # width/height в AnalyzeJob, то есть асинхронно после загрузки. Решение,
  # что объявлять вместо этого, остаётся за вызывающим — молча подставить
  # рамку значило бы вернуть ту же ошибку под видом точности.
  ...
  # Когда блоб ещё не проанализирован и фактических размеров нет —
