...
⚠️ **Таблица выше — про openclaw-машину.** Вторая машина — прод-хост `victory` (`hostname`), и там живут пять своих worktree (`victory`, `-victory`, `-chat`, `-seo`, `-upgrade`): схема «4 сессии» из `.claude/sessions/README.md` на нём не историческая, а рабочая. Различай машины по имени хоста, а не по пути: относительный путь и `~` одинаковы на обеих, и правило «тут можно гонять Ruby» по ним не читается. Из тулинга сессий абсолютные пути к чекауту остались только в `.mcp.json` и в deny-глобах `.claude/settings.json` (там они сопоставляются буквально) — оба ведут на прод-хост `victory`, поэтому на openclaw MCP `postgres` и `rails-guides` не поднимаются (это сломанный путь, а не отсутствующая возможность), а deny-правило на main checkout там инертно. Остальные абсолютные пути — юниты systemd, nginx, крон-строки и runbook'и прод-хоста; полный перечень с цифрами и причинами — в `.claude/memory/techContext.md`, секция «Где абсолютный путь обязателен». Проверяй `git worktree list`, а не память.
