...
- 🚨 **Зависимые части едут стеком PR, а не одним большим PR.** Обязательно, если верно любое из двух: (а) работа делится на слои, где следующий не собирается без предыдущего — миграция → сервис → UI; (б) diff перевалил ~500 строк или ~10 файлов **и работа при этом делится на слои**. PR #16 (2532 строки, 29 файлов) — ровно этот случай. Крупное, но неделимое — например PR #27, цельный сервис на 3133 строки — стеком не режется: везём одним PR и пишем в описании, почему.
  Инструмент — `gh stack`, правила в skill `gh-stack`: `gh stack init <ветки снизу вверх>` → `gh stack submit` → `gh pr ready <PR нижнего слоя>` → после ревью `gh stack merge <номер PR>` → `gh stack sync`. Ручная цепочка `gh pr create --base` **стек на GitHub не создаёт** — выходят несвязанные PR, а `sync` заменяет весь ручной `rebase --onto` + `pr edit --base`.
  Ревью остаётся обязательным — отдельно на каждый PR стека, снизу вверх.
  ⚠️ Верхние слои держим **черновиками** и снимаем черновик по одному, по мере ревью. `submit --open` помечает готовыми **все** PR разом, а `gh stack merge` из проверок смотрит только «открыт и не черновик» — вместе с `merge` без номера PR это отправляет неотревьюенные слои прямо в прод.
  ⚠️ Без TTY `submit` даёт автозаголовки и **пустое описание**: обязательный футер PR не проставится, дописывать `gh pr edit <N> --body`.
  🚨 **Не проверено и, похоже, сломано здесь:** шаг 3 в `sync` — fast-forward trunk, а `main` занят prod-чекаутом `/home/q/victory`. Из любого другого worktree `git fetch origin main:main` отвечает `refusing to fetch into branch 'refs/heads/main' checked out at '/home/q/victory'` (проверено 07.09.26). Значит `sync`, `rebase` и `trunk` в этом окружении либо упадут, либо сдвинут `main` под живым прод-чекаутом. **До первого боевого стека это надо проверить руками** на выброшенной цепочке.
